...
test("Check routes", () => {
  ...
  expect(Routes.registerUser({registerToken: "", username: "", password: ""}).needsAuth).toBe(false);
  expect(Routes.registerUser({registerToken: "", username: "", password: ""}).method).toBe("POST");
  ...
  expect(Routes.createUser({email: "", firstName: "", lastName: ""}).needsAuth).toBe(true);
  expect(Routes.createUser({email: "", firstName: "", lastName: ""}).method).toBe("POST");
  expect(Routes.assignPlanToUser({username: "", planId: ""}).needsAuth).toBe(true);
  expect(Routes.assignPlanToUser({username: "", planId: ""}).method).toBe("POST");
  ...
  expect(Routes.saveTrainingPlan({id: 0, exercise: [{id: 0, date: "", sets: 1, repeats_per_set: 1}], name: ""}).method).toBe("POST");
  expect(Routes.saveTrainingPlan({id: 0, exercise: [{id: 0, date: "", sets: 1, repeats_per_set: 1}], name: ""}).needsAuth).toBe(true);
});
