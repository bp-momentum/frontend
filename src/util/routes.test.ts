...
test("Check routes", () => {
  ...
  expect(Routes.getExercise({ id: 0 }).method).toBe("POST");
  expect(Routes.getExercise({ id: 0 }).needsAuth).toBe(false);
  ...
  expect(Routes.getDoneExercises().needsAuth).toBe(true);
  expect(Routes.getDoneExercises().method).toBe("GET");
